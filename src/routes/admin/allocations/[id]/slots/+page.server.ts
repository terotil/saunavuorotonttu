...
export const actions = {
	add: async ({ request, params, platform }) => {
		...
		if (!canEditSlots(allocation.status)) return fail(403, { addError: 'Vuoroja ei voi muokata' });
		...
		if (startTime >= endTime)
			return fail(400, { addError: 'Lopetusajan on oltava aloitusajan jälkeen' });
	...
	remove: async ({ request, params, platform }) => {
		const allocation = await loadEditable(platform!.env.DB, params.id!);
		if (!canEditSlots(allocation.status)) return fail(403, { removeError: 'Vuoroja ei voi muokata' });
	...
	copy: async ({ request, params, platform }) => {
		const allocation = await loadEditable(platform!.env.DB, params.id!);
		if (!canEditSlots(allocation.status)) return fail(403, { copyError: 'Vuoroja ei voi muokata' });

		const data = await request.formData();
		const sourceId = (data.get('source_id') as string)?.trim();
		if (!sourceId) return fail(400, { copyError: 'Valitse lähdevarauskausi' });
		if (sourceId === params.id) return fail(400, { copyError: 'Lähdettä ei voi olla sama varauskausi' });

		const db = platform!.env.DB;
		const sourceSlots = await getSlotsForAllocation(db, sourceId);
		if (sourceSlots.length === 0) return fail(400, { copyError: 'Lähteessä ei ole vuoroja' });
	...
	generate: async ({ request, params, platform }) => {
		...
		if (!canEditSlots(allocation.status))
			return fail(403, { generateError: 'Vuoroja ei voi muokata' });
		...
		if (!startTime || !/^\d{2}:\d{2}$/.test(startTime))
			return fail(400, { generateError: 'Anna aloitusaika muodossa HH:MM' });
